...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Cryocoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"cryocoin.com", "174.140.169.114"},
    {"cryocoin.org", "209.208.97.65"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Cryocoin is probably already running."), addrBind.ToString().c_str());
        else
