...
#if defined(NDEBUG)
# error "Cryocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc80b8cee940a38e1c7ebbd4dd97dcd5fc0809b69c128846b1cab84737d6e1d05");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cryocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cryocoin Signed Message:\n";
...
{
    // Cryocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Cryocoin: 420k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Cryocoin: 2 days
static const int64 nTargetSpacing = 3 * 60; // Cryocoin: 3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cryocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cryocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Cryocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Robin Williams dead at 63";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x614c0bee40f7121a3b96407e4155222d4af3f19f1cb8d21f42ec0848e29a971f"));	
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Cryocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryocoinMiner(CWallet *pwallet)
{
    printf("CryocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cryocoin-miner");
    ...
    try { loop {
        ...
        printf("Running CryocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryocoinMiner, pwallet));
}
